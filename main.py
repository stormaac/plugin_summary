    ...
    def _check_tokens(self, records, max_tokens=3600):
        ...
        for record in records[::-1]:
            ...
            if is_triggered:
                sentence += " <T>"
            query += "\n\n"+sentence
        prompt = "你是一位群聊机器人，任务是对以下聊天记录进行简明扼要的总结。总结应采用列表形式，并涵盖以下内容：核心讨论话题、重要提问和回答、关键决策和行动项。聊天记录格式说明： - "[x]" 表示 emoji 表情、图片或声音文件的说明 - 消息末尾的 "<T>" 表示该消息触发了机器人的回复，通常是提问或特殊符号（如 "#" 和 "$"），这些消息的权重较低。请不要在回复中包含聊天记录格式中出现的符号。\n"
